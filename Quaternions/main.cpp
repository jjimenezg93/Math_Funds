...
void HandleMouseMovement(int x, int y) {
	euler.yaw = -(static_cast<double>(x) - camera.screenwidth / 2);
	euler.pitch = -(static_cast<double>(y) - camera.screenheight / 2);
	updateEulerOrientation(euler);
}

int main(int argc,char **argv)
{
	//higher screen makes it easier to find balls
...
void Display(void)
{
   ...
   //gluLookAt(camera.position.x,camera.position.y,camera.position.z, target.x , target.y, target.z, camera.up.x,camera.up.y,camera.up.z);
	...
	VECTOR3D newTarget = Add(camera.position, getForward(euler));

    // TODO
	MATRIX4 lookAtMatrix = lookAt(camera.position, newTarget, camera.up);
